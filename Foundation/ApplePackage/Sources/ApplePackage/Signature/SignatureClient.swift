...
public final class SignatureClient: SignatureClientInterface {
    ...
    public func appendMetadata(item: StoreResponse.Item, email: String) throws {
        let archive = try Archive(url: URL(fileURLWithPath: filePath), accessMode: .update)
        ...
        try? fileManager.removeItem(at: metadataUrl.deletingLastPathComponent())
    }
    ...
    private func readPlist<T: Decodable>(archive: Archive, matchingSuffix: String, type: T.Type) throws -> T {
        ...
        let url = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent(UUID().uuidString).appendingPathExtension("plist")
